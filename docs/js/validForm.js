...
jQuery.fn.extend({
  validForm: function validForm(options) {
    var options = $.extend({
      classNok: false,
      classOk: false,
      required: [{
    ...
    function errorFields(config, $form) {
      ...
      if ($form.find('.' + errClass).length) {
        ...
        var $errorTag = $('<' + config.errorTagName + '></' + config.errorTagName + '>').css('display', 'none').addClass(errClass).addClass(config.errorClassName ? config.errorClassName : '').html(config.errorMessage);
    ...
    function checkField($input, config) {
      ...
      if (validValue || func) {
        ...
        if (config.fieldParentClass) {
          $input.parents(config.fieldParentClass).removeClass(options.classNok || '').addClass(options.classOk || '');
        } else {
          $input.removeClass(options.classNok || '').addClass(options.classOk || '');
        }
        ...
        if (config.fieldParentClass) {
          $input.parents(config.fieldParentClass).removeClass(options.classOk || '').addClass(options.classNok || '');
        } else {
          $form.find(config.selector).removeClass(options.classOk || '').addClass(options.classNok || '');
        }
      ...
      $(this).on('submit', function (event) {
        ...
        var errors = options.required.map(function (item) {
          if ($form.find(item.selector).length) {
            ...
            chekErrors = function chekErrors(err) {
          return err === true;
        };
